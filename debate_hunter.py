...
def get_auth_header():
    ...
    # 415エラーを回避するため Content-Type を明示的に追加
    return {
...
# --- 3. AI編集長による企画・記事作成 (本質戦略：論点整理特化) ---

def generate_article_plan(item):
    print(f"   🧠 AI編集長が企画中 (Model: {MODEL_NAME} / 論点整理モード): {item['title'][:20]}...")
    ...
    1. **タイトル (title)**: 
       ...
       - (例: 「〇〇が遂に実写化！キャスト発表でネットは賛否両論、あなたはアリ？ナシ？」)
    ...
    4. **豆知識 (fact_text)**:
       - Wiki資料がある場合はそれを活用し、ない場合でもあなたの知識を活用して、ニュースに関連する業界の裏話や過去の類似事例などを**200文字前後**で詳しく書いてください。
       - **もしどうしても書くことがない場合のみ、空文字 "" にしてください。その場合、見出し(fact_h3)も空文字にしてください。**

    5. **コメント (comments)**:
       - 上記の「コメント生成指示」に従い、5つの異なる視点（賛否両論）のコメントを生成してください。

    【出力形式(JSONのみ)】
    {{
      "category": "social/food/tech/anime/entame/game のいずれか",
      "title": "対立構造がわかるニュース事実＋問いかけ",
      "h2_title": "「〇〇を巡る賛否の声」など、議論を促す導入見出し",
