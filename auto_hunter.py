...
# 情報源リスト (4Gamer と 電ファミニコゲーマー)
RSS_URLS = [
    "https://www.4gamer.net/rss/index.xml",
    "https://news.denfaminicogamer.jp/feed"
]

# 除外ワード (4Gamerはハードウェア情報なども流れるので調整)
NG_WORDS = ["グッズ", "コラボ", "キャンペーン", "イベント", "ポップアップ", "フェア", "記念", "主題歌", "サントラ", "レポート", "舞台", "オーディション", "セール", "ゲーミングPC", "キーボード", "マウス", "ヘッドセット", "チェア", "インタビュー"]
...
def extract_release_date(text):
    ...
    if match_short:
        try:
            ...
            year = now.year
            # 記事の日付が現在より大きく過去（例:1月なのに12月）なら去年の可能性などを考慮できるが、
            # 基本的に新作ニュースなので「今年のその月日」とする
...
<p>{description}</p>
<p><a href="{link}" target="_blank" rel="noopener">ニュースソースで全文を読む</a></p>
"""
        ...
        try:
            ...
            for entry in feed.entries:
                ...
                if not any(w in title for w in TARGET_WORDS):
                    # ログが多すぎると見づらいので、ターゲット外は表示省略してもいいが、今回は確認のため表示
                    # print(f"⚪️ スキップ(対象外): {title[:20]}...") 
                    continue
