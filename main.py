...
1. extract_pure_keyword: プロンプト末尾を「答え:」で終わらせて直接回答を誘導
1. analyze_and_extract_core: JSON形式を明示して直接JSON出力を誘導
1. generate_article_content: 同様にJSON誘導方式に変更
1. select_best_topics: 最後の行からカンマ区切りを取得
1. SyntaxError修正: f文字列内のトリプルクォートを排除
1. 全機能維持
   “””
...
prompt = (
    ...
    "歌手・アーティストはWikiにディスコグラフィがあればWHICH_BEST。\n"
    "キャラ・人物は好感度をLIKE_DISLIKEか推し投票をWHICH_BEST。\n"
...
if text:
    ...
    if result and 'article_type' in result:
        print(f" -> [{result['article_type']}] {result.get('proposed_title', '')}")
        print(f"      👀 意図: {result.get('reason', '')}")
        return result
...
“4. “ + str(count) + “件のうち2〜3件は >>数字 の形で返信を含めること。\n”
“5. 返信する場合は必ず返信元のコメント内容を踏まえた関連性のある内容にすること。\n”
“6. >>数字 の数字は、必ずそのコメント自身の番号より小さい数字にすること。\n”
...
prompt = (
    "トレンドテーマ「" + theme + "」について、読者参加型の「投票記事」をJSON形式で作成してください。\n\n"
    ...
    '"h2_title": "導入H2見出し", "h2_text": "導入文400〜500文字", '
...
if text:
    result = parse_json_from_text(text)
    if result:
        print(f" -> 完了 (コメント{len(result.get('comments', []))}件生成)")
        return result
...
```
    ...
    if len(items_str) == 2:
        sc = (
            f'[vote_bar name_a="{items_str[0]}" name_b="{items_str[1]}"]\n\n'
            f'[vote_summary name_a="{items_str[0]}" name_b="{items_str[1]}"]'
        )
    else:
        sc = (
            f'[vote_bar items="{", ".join(items_str)}"]\n\n'
            f'[vote_summary items="{", ".join(items_str)}"]'
        )
